...
/**
 * Opening a PR fires the fire-and-forget ensure_repo_snapshot with the PR's
...
const recordedEnsures = (page: Parameters<typeof setupApp>[0]) =>
  page.evaluate(
    () =>
      JSON.parse(localStorage.getItem("e2e:snapshotEnsures") ?? "[]") as Record<
        string,
        unknown
      >[]
  );

test("opening a PR ensures a snapshot for its head SHA", async ({ page }) => {
  await setupApp(page);
